        ...
        <div className="bg-white rounded-lg shadow-sm p-8">
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  <th className="text-left px-6 py-4 font-semibold text-gray-700">Ticket ID</th>
              ...
              <tbody>
                {tickets.map((ticket) => (
                  <tr key={ticket.id} className="border-b border-gray-200 hover:bg-gray-50 transition cursor-pointer">
                    ...
                    <td className={`px-6 py-4 text-sm ${getPriorityColor(ticket.priority)}`}>
                      {ticket.priority}
                    </td>
                    <td className="px-6 py-4 text-gray-600 text-sm">{ticket.created}</td>
                    <td className="px-6 py-4 text-gray-600 text-sm">{ticket.updated}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          {tickets.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-400 text-sm mt-2">Create a new ticket to get support.</p>
            </div>
          )}
          ...
          {tickets.length > 0 && (
            <div className="mt-12 pt-8 border-t border-gray-200">
              <h2 className="text-xl font-bold text-gray-800 mb-6">Latest Ticket Details</h2>
              <div className="bg-gray-50 p-6 rounded-lg">
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    <p className="text-xs font-semibold text-gray-500 uppercase">Ticket ID</p>
                    <p className="text-gray-800 font-semibold mt-1">{tickets[0].id}</p>
                  </div>
                  <div>
                    <p className="text-xs font-semibold text-gray-500 uppercase">Status</p>
                    <span className={`inline-block px-3 py-1 rounded-full text-xs font-semibold mt-1 ${getStatusColor(tickets[0].status)}`}>
                      {tickets[0].status}
                    </span>
                  </div>
                </div>
                <div className="mb-4">
                  ...
                  <p className="text-gray-800 font-semibold mt-1">{tickets[0].subject}</p>
                </div>
                <div>
